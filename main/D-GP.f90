module constants
    implicit none
    integer, parameter::dim=3,simu_IC=10000,simu_OC=2000,n=20,window_size=50,ewma=50,level=25
...
program main
    ...
    use Chartingmodule
    implicit none
    real lambdaewma,limit,arl,std,limitl,limitR,CL
    ...
    call D_CLget(dim,nu_matrix,CL)
    limitl=0.3
    limitR=0.8
    call D_limitsearch(lambdaewma,pro_tensor_IC,CL,limitL,limitR,Limit,arl,std)  
    write(10,*),"Limit is",Limit
    write(10,*),"IC_arl is",arl
    write(10,*),"IC_std is",std
    call D_Poi(lambdaewma,nu_matrix,theta_matrix,0.05,-0.10,limit,CL)
    call cpu_time(dClkFinish)  
    write(10,*) "Time is", dClkFinish-dClkStart, "seconds" 
    stop
end program main

subroutine D_Poi(lambdaewma,nu_matrix,theta_matrix,increment_meanshift,increment_covshift,limit,CL)
    use IMSL
    use constants
    use Chartingmodule
    implicit none
    ...
    do i = 1, 9, 1
        ...
        call D_OCARL(lambdaewma,pro_tensor_IC,pro_tensor_OC,CL,Limit,arl,std)
...
subroutine D_OCARL(lambdaewma,pro_tensor_IC,pro_tensor_OC,CL,Limit,arl,std)
  use constants
  use IMSL
    implicit none
    integer i,j
    ...
    Test=CL    
    do j = 1, ewma, 1
      call samplegenerationGP(n,pro_tensor_IC,sample)  
    ...
    do while (Test<CL+Limit .and. Test>CL-Limit)
      call samplegenerationGP(n,pro_tensor_OC,sample)
      call D_statistic(dim,n,lambdaewma,sample,Test)
...
subroutine D_limitsearch(lambdaewma,pro_tensor_IC,CL,limitL,limitR,Limit,arl,std)
use IMSL
use constants
implicit none
    integer i,j
    real pro_tensor_IC(dim,dim,level),lambda_IC(dim,dim),lambdaewma
