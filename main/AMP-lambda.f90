...
program main
    ...
    call AMP_Poi(0.05,0.10,lambdaewma,limit,lambda_0) 
    ...
    write(10,*),"Limit is",Limit
    write(10,*),"IC_arl is",arl
    call AMP_Poi(0.04,0.10,lambdaewma,limit,lambda_0)
	!stop 
...
implicit none
  ...
  enddo 
  std=sqrt(st/(simu_OC-1.0))
  std=std/(sqrt(dble(simu_OC)-1)) 
  return 
...
implicit none
  ...
  !stop
   ...
   do j=1,simu_IC
    st=st+(simuarl(j)-arl)**2.0
  enddo 
  std=sqrt(st/(simu_IC-1.0))
  std=std/(sqrt(dble(simu_IC)-1)) 
  return 
