module constants
    implicit none
    integer, parameter::n=20,simu_time=10000,dim=4,window_size=50
...
program main
    ...
    use Chartingmodule
    implicit none
    real max_time,min_time,ave_time,std_time,lambdaewma
    real c_parameter,lambda_0(dim,dim)
    real dClkStart, dClkFinish
    open(10,file="results/AMP_P4_time.txt")
    lambdaewma=0.10
    call cpu_time(dClkStart)
    c_parameter=0.5 
    call parameter_setting(c_parameter,lambda_0)   
    call AMP_Time_count(lambdaewma,lambda_0,max_time,min_time,ave_time,std_time) 
...
subroutine AMP_Time_count(lambdaewma,lambda_0,max_time,min_time,ave_time,std_time)
use IMSL
use Chartingmodule
use constants
implicit none
  ...
  enddo 
  std_time=sqrt(st/(simu_time))
  std_time=std_time/(sqrt(dble(simu_time))) 
  return 
end subroutine AMP_Time_count
