...
program main
	...
	lambdaewma=0.10
    open(10,file="./results/AMP_WP.txt")
    do i = 1, dim, 1
        do  j=1,dim,1
...
implicit none
    ...
    call samplegenerationWP(n,pro_tensor_IC,sample(j,:,:))
    end do
    ...
    call samplegenerationWP(n,pro_tensor_OC,sample(window_size,:,:))
    call LRT_test(dim,window_size,n,lambdaewma,sample,lambda_IC,Test)
    !print*,Test
...
implicit none
    ...
    do while (Test<Limit)
    sample(1:window_size-1,:,:)=sample(2:window_size,:,:)
    call samplegenerationWP(n,pro_tensor_IC,sample(window_size,:,:))
    call LRT_test(dim,window_size,n,lambdaewma,sample,lambda_IC,Test)
    !print*,Test
    simuarl(i)=simuarl(i)+1.0
    end do
