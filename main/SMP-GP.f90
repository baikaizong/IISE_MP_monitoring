...
program main
    ...
    open(10,file="./results/SMP_GP.txt")
    do i = 1, dim, 1
        do  j=1,dim,1
    ...
    limitl=1.0
    limitR=1.4
    call SMP_limitsearch(lambdaewma,pro_tensor_IC,miu_exp,Cov_exp,limitl,limitR,Limit,arl,std)  
    write(10,*),"Limit is",Limit
    write(10,*),"IC_arl is",arl
    write(10,*),"IC_std is",std
    call SMP_Poi(lambdaewma,nu_matrix,theta_matrix,-0.05,0.10,limit,miu_exp,Cov_exp)
    call cpu_time(dClkFinish)  
    write(10,*) "Time is", dClkFinish-dClkStart, "seconds" 
    stop
end program main

subroutine SMP_Poi(lambdaewma,nu_matrix,theta_matrix,increment_meanshift,increment_covshift,limit,miu_exp,Cov_exp)
    use IMSL
    use constants
    use Chartingmodule
    implicit none
    ...
    do i = 1, 8, 1
        ...
        call SMP_OCARL(lambdaewma,pro_tensor_IC,pro_tensor_OC,miu_exp,Cov_exp,Limit,arl,std)
...
subroutine SMP_OCARL(lambdaewma,pro_tensor_IC,pro_tensor_OC,miu_exp,Cov_exp,Limit,arl,std)
use IMSL
use constants
implicit none
    integer i,j
    ...
    call MM_EstimationII(dim,n,lambdaewma,sample,miu_ewma,Cov_ewma)
...
subroutine SMP_limitsearch(lambdaewma,pro_tensor_IC,miu_exp,Cov_exp,limitl,limitR,Limit,arl,std)
use IMSL
use constants
implicit none
    integer i,j
